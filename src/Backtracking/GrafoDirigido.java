...
public class GrafoDirigido<T> implements Grafo<T> {
	...
	private LinkedHashMap<String, ArrayList<Arco<T>>> vertices;
	...
	public void agregarArco(String verticeId1, String verticeId2, T distancia) {
		if(this.contieneVertice(verticeId1) && this.contieneVertice(verticeId1)) {
			if(!this.existeArco(verticeId1, verticeId2)) {
				Arco<T> nuevo= new Arco<T>(verticeId1, verticeId2, distancia);
	...
	public Iterator<String> obtenerVertices() {
	    ArrayList<String> listaVertices = new ArrayList<>(this.vertices.keySet());
		...
		if(arcos!=null) {
			for(int i=0; i<arcos.size(); i++) {
				if((arcos.get(i).getVerticeOrigen()==verticeId1)&&(arcos.get(i).getVerticeDestino()==verticeId2)) {
					Arco<T> arco= new Arco<T>(verticeId1, verticeId2, arcos.get(i).getDistancia());
					return arco;
				}
			}
		}
